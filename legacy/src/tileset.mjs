...
export class TileSet {
    ...
    removeTileType(tid) {
		...
		// [TODO] remove information about tile type's blockers
    ...
    addTileType(type) {
		...
		for (const dir of DIRECTIONS) {
		    ...
		    if (glue === undefined || glue === null || glue.strength === 0) {
				...
				if (glue.blockertemp != null) {
					const blockerEntry = {tileTypeName: type.name, blockedSide: dir, strength: glue.strength, blockerTemp: glue.blockertemp};
					if (glue.blockertemp in this.tileGlueBlockers) {
						this.tileGlueBlockers[glue.blockertemp].push(blockerEntry);
					} else {
						this.tileGlueBlockers[glue.blockertemp] = [blockerEntry];
					}
				}
		    }
		}
    ...
    fromTDS(tdsData) {
		...
		for (let line of lines) {
			...
			type = {
			    ...
			    glues: {
					north: { label: "", strength: 0, blockertemp: null },
					east:  { label: "", strength: 0, blockertemp: null },
					south: { label: "", strength: 0, blockertemp: null },
					west:  { label: "", strength: 0, blockertemp: null },
			    },
			};
		    } else if (cmd === "label") {
			type.label = arg;
