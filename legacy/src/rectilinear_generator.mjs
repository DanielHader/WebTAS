export class TileType {
    ...
    constructor(name = '', label = '', color = '', glues = null) {
        this.name = name;
        this.label = label;
        this.color = color;
        if (glues == null) {
    ...
    outputString() {
        let out = 'TILENAME ' +  this.name + '\n';
        out += 'LABEL ' + this.label + '\n';
        out += 'TILECOLOR ' + this.color + '\n';
        if ((this.glues.north.strength > 0) && (this.glues.north.label.length > 0)) {
            out += 'NORTHBIND ' + this.glues.north.strength + '\n';
            out += 'NORTHLABEL ' + this.glues.north.label + '\n';
        }
        if ((this.glues.east.strength > 0) && (this.glues.east.label.length > 0)) {
            out += 'EASTBIND ' + this.glues.east.strength + '\n';
            out += 'EASTLABEL ' + this.glues.east.label + '\n';
        }
        if ((this.glues.south.strength > 0) && (this.glues.south.label.length > 0)) {
            out += 'SOUTHBIND ' + this.glues.south.strength + '\n';
            out += 'SOUTHLABEL ' + this.glues.south.label + '\n';
        }
        if ((this.glues.west.strength > 0) &&  (this.glues.west.label.length > 0)) {
            out += 'WESTBIND ' + this.glues.west.strength + '\n';
            out += 'WESTLABEL ' + this.glues.west.label + '\n';
        }
        out += 'CREATE\n\n';
        return out;
    }
...
export class SimpleDataTable {
    ...
    _renderTBody($table) {
        ...
        this.data.forEach((item, rowIndex) => {
            ...
            Object.entries(item).forEach(([key, value]) => {
				let bReadOnly = (count < 2) ? true : false;
                const $cell = this._createCellWithInput(key, value, rowIndex, bReadOnly);
                $row.appendChild($cell);
				count = count + 1;
            });
    ...
    _createCellWithInput(name, value, rowIndex, bReadOnly) {
        const $cell = document.createElement('td');
        const $input = document.createElement('input');
        $input.value = value;
        $input.name = name;

        if (this.readonly || bReadOnly) {
            $input.disabled = true;
        }
    ...
    _createEmptyRow() {
        ...
        columnNames.forEach((cellName) => {
            const $cell = this._createCellWithInput(cellName, '', rowsCount, false);
            $row.appendChild($cell);
            record[cellName] = '';
        });
